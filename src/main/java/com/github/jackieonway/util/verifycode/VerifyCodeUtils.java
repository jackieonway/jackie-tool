package com.github.jackieonway.util.verifycode;
...
public enum  VerifyCodeUtils {
    ...
    public static String generateVerifyCode(int w, int h, File outputFile, int verifySize) throws IOException {
        ...
        generateVerifyCode(w, h, outputFile, verifyCode);
        return verifyCode;
    }
    ...
    public static String generateVerifyCode(int w, int h, OutputStream os, int verifySize) throws IOException {
        String verifyCode = generateVerifyCode(verifySize);
        generateVerifyCode(w, h, os, verifyCode);
        return verifyCode;
    }
    ...
    public static void generateVerifyCode(int w, int h, File outputFile, String code) throws IOException {
        ...
        generateVerifyCode(w, h, fos, code);
        fos.close();
    }
    ...
    public static void generateVerifyCode(String sessionCodeId, String code, String result,
        ...
        for (int i = 0; i < verifySize; i++) {
            ...
            affine.setToRotation(Math.PI / 4 * random.nextDouble() * (random.nextBoolean() ? 1 : -1),
                    (w * 1.0d / verifySize) * i  + fontSize / 2.0d, h / 2.0d);
    ...
    public static Map<String, Object> getChinese(int len) {
        ...
        for (int i = 0; i < len; i++) {
            switch (random.nextInt(2)) {
                case 1:
                    sb.append(VERIFY_CODES.charAt(random.nextInt(VERIFY_CODES.length() - 1)));
                    break;
                case 0:
                default:
                    String ch = BASE_CHINESE.charAt(random.nextInt(BASE_CHINESE.length())) + "";
                    sb.append(ch);
                    break;
            }
        }
